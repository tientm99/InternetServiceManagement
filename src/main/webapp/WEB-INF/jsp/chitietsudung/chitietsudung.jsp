							...
							<tbody class="bg-light">
								<c:forEach var="ctsd" items="${listCTSD}" varStatus="count">
									<tr>
										...
										<td>${ctsd.diaChi}</td>
										<td>${ctsd.maMay}</td>
										<td class="text-right">${ctsd.viTri}</td>
										<td>${ctsd.trangThai}</td>
										<td class="text-center">${ctsd.ngayBatDauSD}</td>
										<td class="text-center">${ctsd.gioBatDauSD}</td>
										<td class="text-right">${ctsd.thoiGianSD}</td>
										<td>${ctsd.maDV}</td>
										<td class="text-center">${ctsd.ngaySD}</td>
										<td class="text-center">${ctsd.gioSD}</td>
										<td class="text-right">${ctsd.soLuong}</td>
										<td class="text-right">${ctsd.tongTien}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
				...
				<div class="row">
					<div class="btn pagination mt-4 d-block col-12"
						...
						<c:if test="${currentPage > 1}">
							<a href="listchitietsudung?page=${currentPage-1}">
						...
						<c:forEach begin="1" end="${totalPages}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<a href="listchitietsudung?page=${i}">
